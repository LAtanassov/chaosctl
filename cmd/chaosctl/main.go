...
func main() {
	addr := flag.String("proxy.address", ":8080", "proxy adress")
	lr := flag.Float64("latency.rate", .33, "latency rate")
	lc := flag.Int64("latency.sec", 10, "latency in seconds")
	br := flag.Float64("block.rate", 0.33, "block rate")
	flag.Parse()
	...
	log.Printf("proxy listens %s", *addr)
